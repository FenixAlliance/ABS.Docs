...
The [**Identity and Access Management** Module ("**IAM**")](~/Modules/IAM.md) is an extension built on top of the Alliance Passport Services Framework to allow users and applications to connect to certain Alliance Business Suite instances.
...
An [Account Holder](~/Capabilities/Alliance-Passport-Service/Account-Holders.md) must be authenticated (signed in to the Alliance Business Suite) using their credentials to send a request to any [Alliance Business Platform](~/Capabilities/Alliance-Business-Platform.md) API. Some modules, such as [Media Portals](~/Modules/CMS.md), ICX Stores, and ABS Workplace, allow a few requests from anonymous users. However, they are the exception to the rule.
...
Users might also be required to provide additional security information. For example, Alliance Business Systems recommends that you use multi-factor authentication (MFA) to increase the security of your account and to enforce it on Privileged Account Holders. To learn more about the IAM entities that the Alliance Passport Service can authenticate, see [APS Identity Holders](~/Capabilities/Alliance-Passport-Service/Account-Holders.md) and [APS Security Roles](~/Capabilities/Alliance-Passport-Service/Security-Roles.md).
