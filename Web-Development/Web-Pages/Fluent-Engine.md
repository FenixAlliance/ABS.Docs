## Welcome to the Fluid Content Authoring Engine Documentation

Fluid Content Authoring Engine is a template engine based on the Liquid template language. It's a secure template language that is also very accessible for non-programmer audiences.




## Features
- Very fast Liquid parser and renderer (no-regexp), with few allocations.
- Secure templates by allow-listing all the available properties in the template. (User templates can't break your Alliance Business Suite).
...
- Register any ABS types and properties, or define custom handlers to intercept when a named variable is accessed.
...
## Using Fluid in your Alliance Business Suite
